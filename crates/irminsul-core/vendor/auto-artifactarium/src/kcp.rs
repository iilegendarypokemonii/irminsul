...
impl KcpSniffer {
    ...
    pub fn receive_segments(&mut self, segments: &[u8]) -> Vec<Vec<u8>> {
        ...
        let Some(segments) = reformat_kcp_segments(segments) else { return Vec::new(); };
        ...
        while let Ok(size) = self.kcp.peeksize() {
            if size > 8 * 1024 * 1024 { return Vec::new(); }
...
fn reformat_kcp_segments(data: &[u8]) -> Option<Vec<u8>> {
    ...
    let mut i = 0;
    while i < data.len() {
        let header = data.get(i..i.checked_add(32)?)?;
        ...
        let end = (i + 32).checked_add(content_len)?;
        let content = data.get(i + 32..end)?;
